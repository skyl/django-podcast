...
class Show(models.Model):
    ...
    language = models.CharField(max_length=5, default='en-us',\
            help_text='Default is American English. See <a href="http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes">ISO 639-1</a> and <a href="http://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements">ISO 3166-1</a> for more language codes.', blank=True)

    copyright = models.CharField(max_length=255, default='All rights reserved', choices=COPYRIGHT_CHOICES,\
            help_text='See <a href="http://creativecommons.org/about/license/">Creative Commons licenses</a> for more information.')
    copyright_url = models.URLField('Copyright URL', blank=True,\
            help_text='A URL pointing to additional copyright information. Consider a <a href="http://creativecommons.org/licenses/">Creative Commons license URL</a>.')
    author = models.ForeignKey(User, related_name='show_authors',\
            help_text='Remember to save the user\'s name and e-mail address in the <a href="../../../auth/user/">User application</a>.<br />')
    webmaster = models.ForeignKey(User, related_name='webmaster', blank=True, null=True,\
            help_text='Remember to save the user\'s name and e-mail address in the <a href="../../../auth/user/">User application</a>.')
    category_show = models.CharField('Category', max_length=255, blank=True,\
            choices = CATEGORY_CHOICES,\
            help_text='Limited to one user-specified category for the sake of sanity.')
    ...
    ttl = models.PositiveIntegerField('TTL',\
            help_text='"Time to Live," the number of minutes a channel can be cached before refreshing.', blank=True, null=True)
    image = models.ImageField(upload_to='podcasts/shows/img/',\
            help_text='An attractive, original square JPEG (.jpg) or PNG (.png) image of 600x600 pixels. Image will be scaled down to 50x50 pixels at smallest in iTunes.', blank=True)
    feedburner = models.URLField('FeedBurner URL',\
            help_text='Fill this out after saving this show and at least one episode. URL should look like "http://feeds.feedburner.com/TitleOfShow". See <a href="http://code.google.com/p/django-podcast/">documentation</a> for more.', blank=True)
    ...
    itunes = models.URLField('iTunes Store URL', help_text='Fill this out after saving this show and at least one episode. URL should look like "http://phobos.apple.com/WebObjects/MZStore.woa/wa/viewPodcast?id=000000000"', blank=True)
